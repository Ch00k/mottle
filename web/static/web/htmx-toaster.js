class ToasterComponent extends HTMLElement {
    ...
    generateTheme() {
        const colors =
            document
                .querySelector('script[data-htmxt-colors]')
                ?.getAttribute('data-htmxt-colors') || null;
        if (!colors) return;
        const colorArray = colors.split(';');
        const defaults = ['#f2f2f2', '#000', '#e2f9ec', '#e6f1fb', '#f8e3e3'];
        const [bg, txt, sc, inf, err] = colorArray.map(
    ...
    initializeEventListener() {
        ...
        document.body.addEventListener('htmx:afterRequest', (event) => {
            ...
            if (body) {
                this.addToast(
                    body,
                    event.detail.xhr.getResponseHeader('HXToaster-Type') ||
                        'default'
                );
            }
        });
    ...
    renderToast(id) {
      ...
      `;
        ...
        toastElement.addEventListener(
            'touchstart',
            (event) => this.handleTouchStart(event, toastElement),
            { passive: true }
        );
        toastElement.addEventListener(
            'touchmove',
        ...
        toastElement.addEventListener(
            'touchend',
            (event) => this.handleTouchEnd(event, id),
            { passive: true }
        );
